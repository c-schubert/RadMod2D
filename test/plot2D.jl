...
function plot_model(fig, ax, m::AbstractModel; show_norm_vec=false, show_nodes=false, 
    ...
    for p = 1:m.no_parts
        ...
        # plot elements
        elem_lines[p] = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i in erange]
        plt_elem[p] = linesegments!(ax, elem_lines[p], color=colors[p], linewidth=linewidth)
        # plot nodes
        if show_nodes == true
            local nodes_points = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) for i in erange]
            append!(nodes_points, [Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i in erange])
            plt_nodes[p] = scatter!(ax, nodes_points, color=colors[p], markersize=10)
        end
        # plot com
        if show_com == true
            local com_points = [Point2f(m.elements[i].com.x, m.elements[i].com.y) for i in erange]
            plt_com[p] = scatter!(ax, com_points, color=:grey, markersize=10)
        end
        # plot norm_vec as linesegments
        if show_norm_vec == true
            local com_points = [Point2f(m.elements[i].com.x, m.elements[i].com.y) for i in erange]
            # local norm_vec_lines = [Point2f(m.elements[i].com.x, m.elements[i].com.y) => 
...
function plot_model_elements(fig, ax, m::AbstractModel, erange; show_norm_vec=false, show_nodes=false, show_com=false, color=:cyan, linewidth=3.0, linestyle=nothing)
    # plot only given elements
    # color = :cyan
    # plot elements
    elem_lines = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
        Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i in erange]
...
function plot_empty_tiles(fig, ax, dx::T1, dy::T1, n::T2; linewidth=3.0) where {T1<:AbstractFloat,T2<:Integer}
    # plot empty tiles
    # plot x lines
    t_lines = [Point2f(i * dx, 0) => Point2f(i * dx, n * dy) for i = 0:1:n]
    plt_elem = linesegments!(ax, t_lines, color=:black, linewidth=linewidth)
    # plot y lines
    t_lines = [Point2f(0, i * dy) => Point2f(n * dx, i * dy) for i = 0:1:n]
...
function plot_points_and_connection(fig, ax, p1::Point2D{T}, p2::Point2D{T}) where {T<:AbstractFloat}
    linesegments!(ax, [Point2f(p1.x, p1.y) => Point2f(p2.x, p2.y)], color=:blue, linewidth=3)
    scatter!(ax, Point2f(p1.x, p1.y), color=:green, markersize=15)
    scatter!(ax, Point2f(p2.x, p2.y), color=:red, markersize=15)
end
...
function plot_existing(fig, ax, m::AbstractModel, e::T, mat; color=:yellow, linewidth=2) where {T<:Integer}
    ...
    # plot rays
    # color = :yellow
    ray_lines = [Point2f(m.elements[e].com.x, m.elements[e].com.y) =>
        Point2f(m.elements[i].com.x, m.elements[i].com.y) for i in exist]
...
function plot_model_shadow_1to1(fig, ax, m::AbstractModel, mat, source, target)
    ...
    source_lines = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
        Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_source]
    # plot target
    ...
    target_lines = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
        Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_target]
    ...
    for p in rest
        ...
        rest_lines[s1:s2] = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_rest]
    end
    # plot it
...
function plot_model_shadow_1toAll(fig, ax, m::AbstractModel, mat, source)
    ...
    for p in target
        ...
        target_lines[s1:s2] = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_target]
        k = 0
        for j = s1:s2
            k += 1
...
function plot_model_with_value(fig, ax, m::AbstractModel, val::Vector{T}, val_label; 
        parts=1:m.no_parts, cmap=:jet, linewidth=5.0, showcbar=true, cbarpos="right") where {T<:AbstractFloat}
    # plot specified parts of model with their values in global colormap
    n = 0
    for p in parts
    ...
    for p in parts
        ...
        elem_lines[s1:s2] = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange]
    end
    plt_elem = linesegments!(ax, elem_lines, color=pval, colormap=cmap, linewidth=linewidth)
    # all at once
    # elem_lines = [Point2f(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) => 
    #             Point2f(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i=1:m.no_elements]
