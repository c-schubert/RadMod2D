...
function fig_mesh(m)    
    # 2D plot
    fig1 = Figure(size = (1000, 1000), font = "Arial", fontsize = fontsize)
    ax1 = fig1[1,1] = Axis(fig1)
    ax1.xlabel = "X in m"
    ax1.ylabel = "Y in m"
    ax1.aspect = DataAspect()
    colors = Vector{Any}(undef,m.no_parts)
    colors[1:4] .= :red
    colors[5:end] .= :blue
    plot_model(fig1, ax1, m, show_norm_vec = false, show_nodes = false, show_com = false, show_leg = false, leg_pos = "right", linewidth = linewidth, colors = colors)
    # display(fig1)
...
function calc_q(m)
    ...
    fig2 = Figure(size = (1000, 1000), font = "Arial", fontsize = fontsize)
    ax2 = fig2[1,1] = Axis(fig2)
    ax2.xlabel = "X in m"
    ax2.ylabel = "Y in m"
    ax2.aspect = DataAspect()
    cbar2 = plot_model_with_value(fig2, ax2, m, temp[:,1] .- 273.15, "Temperatur in °C", cmap = :jet, showcbar = true, cbarpos = "right", linewidth = linewidth)
    cbar2.height = Relative(2/3)
    save("fig_reku_temp2D"*filetype, fig2)
    # 2D plot of heat flux
    fig3 = Figure(size = (1000, 1000), font = "Arial", fontsize = fontsize)
    ax3 = fig3[1,1] = Axis(fig3)
    ax3.xlabel = "X in m"
    ax3.ylabel = "Y in m"
    ax3.aspect = DataAspect()
    cbar3 = plot_model_with_value(fig3, ax3, m, q[:,1] .* (-1), "Wärmestromdichte in W/m", cmap = :jet, showcbar = true, cbarpos = "right", linewidth = linewidth)
